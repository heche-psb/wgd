...
The global MRBHs is constructed by exhaustively merging all the possible pair-wise RBHs except for querying the same sequence itself, which can be initiated by add the flag `-gm` or `--globalmrbh`. The rest of relevant parameters stays the same as the local MRBHs.
