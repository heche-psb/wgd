    ...
    for (c, dist) in zip(colors, args):
        for ax, k, f in zip(axs.flatten(), keys, funs):
            if not nodeaverage:
                ...
                elif funs[1] == f or funs[2] == f: ax.hist(y, bins = np.arange(-4, 1.1, 0.1), weights=w, color=c, alpha=1, rwidth=0.8)
                ...
                elif funs[1] == f or funs[2] == f: ax.hist(y, bins = np.arange(-4, 1.1, 0.1), color=c, alpha=1, rwidth=0.8)
