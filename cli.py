...
def cli(verbosity):
    """
    wgd v2 - Copyright (C) 2026-2027 Hengchi Chen\n
...
@click.option('--chrorder', '-co', type=click.Choice(['alphabetnumber', 'length']), default='alphabetnumber', show_default=True, help='The order of presenting chromosomes/scaffolds')
def viz(**kwargs):
    """
...
def _viz(datafile,spair,outdir,gsmap,plotkde,reweight,em_iterations,em_initializations,prominence_cutoff,segments,minlen,maxsize,anchorpoints,multiplicon,genetable,rel_height,speciestree,onlyrootout,minseglen,keepredun,extraparanomeks,plotapgmm,plotelmm,components,mingenenum,plotsyn,dotsize,apalpha,hoalpha,showrealtick,ticklabelsize,xlim,ylim,adjustortho,adjustfactor,okalpha,focus2all,classic,nodeaveraged,toparrow,bootstrap,gistrb,nthreads,chrorder):
    ...
    if plotsyn:
        ...
        if chrorder == 'alphabetnumber':
            ...
            logging.info("The presenting chromosome/scaffold order follows the order of their lengths, which may not strictly be like chr1, chr2, .. Use --chrorder option if you want to change to order by alphabet and numerical order")
        dotplotingene(ordered_genes_perchrom_allsp,removed_scfa,outdir,table,gene_orders,anchor=df_anchor,ksdf=df,maxsize=maxsize,dotsize=dotsize, apalpha=apalpha, hoalpha=hoalpha,showrealtick=showrealtick, las=ticklabelsize, gistrb=gistrb, alnumchrorder=chrorder)
        ...
        figs = all_dotplots(table, segs, df_multi, minseglen, anchors=df_anchor, maxsize=maxsize, minlen=minlen, outdir=outdir, Ks = df, dotsize=dotsize, apalpha=apalpha, hoalpha=hoalpha, showrealtick=showrealtick, las=ticklabelsize, gistrb=gistrb,alnumchrorder=chrorder)
...
def _syn(families, gff_files, ks_distribution, pathiadhore, outdir, feature, attribute,
        minlen, maxsize, ks_range, iadhore_options, minseglen, keepredun, mingenenum, dotsize, apalpha, hoalpha, additionalgffinfo, showrealtick, ticklabelsize, gistrb, nthreads, chrorder, ancestor=None):
    """
    Co-linearity and anchor inference using I-ADHoRe.
    """
    ...
    if chrorder == 'alphabetnumber':
        ...
        logging.info("The presenting chromosome/scaffold order follows the order of their lengths, which may not strictly be like chr1, chr2, .. Use --chrorder option if you want to change to order by alphabet and numerical order")
    dotplotingene(ordered_genes_perchrom_allsp,removed_scfa,outdir,table,gene_orders,anchor=anchors,ksdf=df_ks,maxsize=maxsize,dotsize=dotsize,apalpha=apalpha, hoalpha=hoalpha, showrealtick=showrealtick, las = ticklabelsize, gistrb=gistrb, alnumchrorder=chrorder)
    ...
    figs = all_dotplots(table, segs, multi, minseglen, anchors=anchors, maxsize=maxsize, minlen=minlen, outdir=outdir, ancestor=ancestor, Ks = df_ks, dotsize=dotsize, apalpha=apalpha, hoalpha=hoalpha, las=ticklabelsize, gistrb=gistrb, chrorder=chrorder) 
    for k, v in figs.items():
        v.savefig(os.path.join(outdir, "{}.dot.svg".format(k)))
        v.savefig(os.path.join(outdir, "{}.dot.pdf".format(k)))
        v.savefig(os.path.join(outdir, "{}.dot.png".format(k)),dpi=500)
